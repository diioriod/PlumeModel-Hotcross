...
! already have axmin in ss
!                kxt(i,j,k) = bit5k * ( .5d0 * ( ss(i,j,k) + ss(i+1,j,k) ) ) / prandtl 
          enddo;enddo;enddo
...
! already include axmin to ss
!                kyt(i,j,k) = bit5k * ( .5d0 * ( ss(i,j,k) + ss(i,j+1,k) ) ) / prandtl
          enddo;enddo;enddo
...
! Aug 2014 corrected the calculation of chi with /2dz /2dx /2dy since difference is over 2 grid cells
            ...
            do j = 1, ny
            do i = 1, nx
               chi_v(i,j,k) = kzt(i,j,k)* ( (tprime(i,j,k-1) - tprime(i,j,k+1))/ (2.0d0*dz) )**2 
            enddo;enddo;enddo
!$OMP END PARALLEL DO
!$OMP PARALLEL DO
            do k = 1, nz
...
! revised March 2015 to take into account an average horizontal Ah in x and y hence Kh
               chi_h(i,j,k) =  kxt(i,j,k) * ( (tprime(i+1,j,k) - tprime(i-1,j,k)) / (2.0d0*dx) )**2
     &                      +  kyt(i,j,k) * ( (tprime(i,j+1,k) - tprime(i,j-1,k)) / (2.0d0*dy) )**2 
            enddo;enddo;enddo
