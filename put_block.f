...
!       tag: subroutine ../vents2/lavelle/NewCross/put_full_block.f - version 6.4.97
    	...
    	subroutine put_full_block_header(nxs,nxe,nys,nye,nzs,nze,output_file_shot,cdf_id)
     ...
     &                    ,grid_id_kxt(4),grid_id_kyt(4),grid_id_kzt(4)
          integer  ind_s,ind_e,start(4),count(4),rcode

          integer, save:: var_id_u,var_id_v,var_id_w,var_id_p,var_id_ss, var_id_ss_v,var_id_temp,var_id_s,var_id_c,
...
!         real * 4 sx_axis(nx+2),sy_axis(nye-nys+1),sz_axis(nz+2)                               !axes data
      ...
      ENTRY PUT_full_block_OPEN(nxs, nxe, nys, nye, nzs, nze, output_file_shot,cdf_id)
          ...
          grid_id_kxt = grid_id_u 
          grid_id_kyt = grid_id_v
          grid_id_kzt = grid_id_w
...
!         ------------------------------------------------------------------------------------------
           ...
           !call load_axis ( cdf_id, axis_id_point,   1,       1.0,    1.0 )
     ...
     &       real ( alphav( nxs: nxe, nys-1: nye), kind =4), rcode)                                   
      ...
      ENTRY PUT_full_block(nxs, nxe, nys, nye, nzs, nze, output_file_shot,cdf_id)

          mct = mct + 1
          write(6,*) 'mct, time in putblock', mct, time
     ...
     &              real ( ss(0:nx+1,nys:nye,0: nz+1 ), kind = 4 ), rcode)
          call ncvpt (cdf_id, var_id_ss_v, (/1, 1, 1, mct/),(/nxe-nxs+1, nye-nys+1, nze-nzs+1, 1/),
     &              real ( ss_v(0:nx+1,nys:nye,0: nz+1 ), kind = 4 ), rcode)
          call ncvpt (cdf_id, var_id_epsh, (/1, 1, 1, mct/),(/nxe-nxs+1, nye-nys+1, nze-nzs+1, 1/),
     &              real ( epsh(0:nx+1,nys:nye,0: nz+1 ), kind = 4 ), rcode)
          call ncvpt (cdf_id, var_id_epsv, (/1, 1, 1, mct/),(/nxe-nxs+1, nye-nys+1, nze-nzs+1, 1/),
     &              real ( epsv(0:nx+1,nys:nye,0: nz+1 ), kind = 4 ), rcode)
     ...
     &              real ( Ri(0:nx+1,nys:nye,0:nz+1 ), kind = 4 ), rcode)
          call ncvpt (cdf_id, var_id_rho, (/1, 1, 1, mct/),(/nxe-nxs+1, nye-nys+1, nze-nzs+1, 1/),
     &              real ( rrho(0:nx+1,nys:nye,0:nz+1 ), kind = 4 ), rcode)
...
!          call start_n_count ( start, 1, 1, 1, mct, count, nx+2, ny+2, nz+2, 1)
     ...
     &            real ( cc4(0:nx+1,0:ny+1,0: nz+1 ), kind =4 ), rcode)
      ...
      ENTRY PUT_full_block_CLOSE(nxs, nxe, nys, nye, nzs, nze,output_file_shot,cdf_id)
