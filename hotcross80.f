...
!compile!:
!This is Dr. Lavelle's command compilation
...
!This is the command compilation for the GACRC
!pgf95 -mp -r8 -Mextend -fast -Kieee -Minline -Mdclchk -O3 -I/usr/local/netcdf/4.1.3/pgi123/include modules80.f hotcross80.f source80.f smagorinsky_split_mixing.f potential_density80.f initialize_profiles80.f hs3crt_composite80.o mixing80.f body_force80.f sponge80.f mpdata-hotcross80.f write_snapshota.f view_3d.f view_2d.f stat_2d.f stat_3d.f -L/usr/local/netcdf/4.1.3/pgi123/lib -L/usr/bin -lnetcdf -lnetcdff -lcurl -o hotcross -mcmodel=medium
...
!   write.inc 
        ...
        integer getcwd
               ...
               wdate_t = '        '; wtime_t = '          ';zone_t = '     '
               call date_and_time(wdate_t,wtime_t,zone_t,tvalues)
                     wdate(1) = wdate_t; wtime(1)=wtime_t

         call OMP_set_num_threads(6)
!        if (nx .eq. 192) call OMP_set_num_threads(6)

!$omp PARALLEL

        if ( OMP_get_thread_num() .eq. 0) then

           write(6,*) 'number of threads being used: ', OMP_get_num_threads()
...
d!              idum = setvbuf3f(65,2,10)
     ...
     &            stop 'side_slip must be either -1(no tangential vel along sides) or 1 (free slip along sides)'
       ...
       open( unit = 81 , status = 'unknown', file = 'cfl.out')
             idum = setvbuf3f(81,2,10)
       open(unit=411,  status = 'unknown')
d           idum= setvbuf3f(411,2,10) 
       open( unit = 401 , status = 'unknown', file = 'temp.diagnostics')
d           idum = setvbuf3f(401,2,10)
       open( unit = 402 , status = 'unknown', file = 'salt.diagnostics')
d           !idum=  setvbuf3f(402,2,10)
       open( unit = 403 , status = 'unknown', file = 'tracer.diagnostics')
...
! --------------------------------------------------------------------------------
      ...
      ENTRY  CFL_VALUES
          ...
          if (max(maxu,maxv,-minu,-minv,maxw,-minw) .gt. 0.5) then
...
!$OMP PARALLEL DO
       ...
       do  i = 1, nx
             ...
             wp(i,j,k) = (1 - delta) * w(i,j,k) + delta *wm(i,j,k) + leap * dt * ( -  (p(i,j,k+1) - p(i,j,k)) / dz + fz(i,j,k) )
