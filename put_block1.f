...
!       tag: subroutine ../vents2/lavelle/NewCross/put_full_block1.f - version 6.4.97

    	subroutine push_full_block1_header(nxs,nxe,nys,nye,nzs,nze,output_file_shot,cdf_id)
     ...
     &           var_id_cc1,var_id_cc2,var_id_cc3,var_id_cc4,var_id_az, var_id_alphae, var_id_alphau, var_id_alphav
          ...
          !real * 4 work_f ( (nx+3)*(nye-nys+2)*(nz+3) )
          !real * 4 ux_axis(nx+3), wz_axis(nz+3), vy_axis(nye-nys+2)
          !real * 4 sx_axis(nx+2),sy_axis(nye-nys+1),sz_axis(nz+2)                               !axes data
      ...
      ENTRY PUT_full_block1_OPEN(nxs, nxe, nys, nye, nzs, nze, output_file_shot,cdf_id)
...
!         ------------------------------------------------------------------------------------------
           ...
           call load_axis ( cdf_id, axis_id_sy,  nye-nys +1,  sy_start,   dy_s )
           call load_axis ( cdf_id, axis_id_sz,  nz +2,  sz_start,   dz_s )

           call load_axis ( cdf_id, axis_id_ux,  nx +3,  ux_start,   dx_s )
           call load_axis ( cdf_id, axis_id_vy,   nye-nys+2,  vy_start,   dy_s )
     ...
     &       real ( alphav( nxs: nxe, nys-1: nye), kind =4), rcode)                                                                                              
      ...
      ENTRY PUT_full_block1(nxs, nxe, nys, nye, nzs, nze, output_file_shot,cdf_id)
...
!         call start_n_count ( start, 1, 1, 1, mct, count, nx+3, nye-nys+1, nz+2, 1)
!         write(6,*) nys, nye,nye-nys+1,start,count
          call ncvpt (cdf_id, var_id_u,(/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/) , 
     &              real ( u(-1: nx+1, nys:nye, 0: nz+1 ), kind = 4 ), rcode)

          endif

          if (toggle(2) .eq. 1) then
          !call start_n_count ( start, 1, 1, 1, mct, count, nx+2, nye-nys+2, nz+2, 1)
          !call ncvpt (cdf_id, var_id_v, start, count, real ( v(0: nx+1,-1: ny+1,0: nz+1 ), kind = 4 ), rcode)
          call ncvpt (cdf_id, var_id_v, (/1, 1, 1, mct/),(/nx+2, nye-nys+2, nz+2, 1/), 
     &      real ( v(0: nx+1, nys-1: nye,0: nz+1 ), kind = 4 ), rcode)
          endif
          ...
          call ncvpt (cdf_id, var_id_w, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &         real ( ww(0:nx+1,nys:nye,-1: nz+1 ), kind = 4 ), rcode)
          endif


          if (toggle(4) .eq. 1) then

          call ncvpt (cdf_id, var_id_p, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &              real( p(0:nx+1,nys:nye,0:nz+1 ), kind = 4 ), rcode)

          endif

          if (toggle(5) .eq. 1) then
          call ncvpt (cdf_id, var_id_ss, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &              real ( ss(0:nx+1,nys:nye,0: nz+1 ), kind = 4 ), rcode)
          endif

          if (toggle(6) .eq. 1) then
          call ncvpt (cdf_id, var_id_temp, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/),
     &               real ( t(0:nx+1,nys:nye,0: nz+1 ), kind = 4), rcode)
          endif


          if (toggle(7) .eq. 1) then
!          call start_n_count ( start, 1, 1, 1, mct, count, nx+2, ny+2, nz+2, 1)
          call ncvpt (cdf_id, var_id_s,(/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &               real ( s(0:nx+1,nys:nye,0: nz+1 ), kind = 4), rcode)
          endif

          if (toggle(8) .eq. 1) then
!    	  call start_n_count ( start, 1, 1, 1, mct, count, nx+2, ny+2, nz+2, 1)
          call ncvpt (cdf_id, var_id_c, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &               real ( c( 0: nx+1,nys: nye,0: nz+1 ), kind = 4), rcode)
          endif

          if (toggle(9) .eq. 1) then
!          call start_n_count ( start, 1, 1, 1, mct, count, nx+2, ny+2, nz+2, 1)
          call ncvpt (cdf_id, var_id_cc1, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/),
     &           real ( cc1(0:nx+1,nys:nye,0: nz+1 ), kind =4 ), rcode)
          endif

          if (toggle(10) .eq. 1) then                                                                                      
!          call start_n_count ( start, 1, 1, 1, mct, count, nx+2, ny+2, nz+2, 1)
          call ncvpt (cdf_id, var_id_cc1, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &                 real ( cc2(0:nx+1,nys:nye,0: nz+1 ), kind =4 ), rcode)
          endif

          if (toggle(11) .eq. 1) then
!          call start_n_count ( start, 1, 1, 1, mct, count, nx+2, ny+2, nz+2, 1)
          call ncvpt (cdf_id, var_id_cc1, (/1, 1, 1, mct/),(/nx+2, nye-nys+1, nz+2, 1/), 
     &               real ( cc3(0:nx+1,nys:nye,0: nz+1 ), kind =4 ), rcode)
          endif
...
!         call start_n_count ( start, 1, 1, 1, 1, count, 1, 1, nz+2, 1)
      ...
      ENTRY PUT_full_block1_CLOSE(nxs, nxe, nys, nye, nzs, nze,output_file_shot,cdf_id)
